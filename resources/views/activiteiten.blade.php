@php
    // $activiteiten en $isLoggedIn komen uit de controller
@endphp
...
<div
    ...
    <a href="{{ url('/') }}" style="font-weight:700; text-decoration:none; font-size:18px;">
        Activiteiten
    </a>
    ...
    <div>
        ...
        @else
            <a href="{{ route('login') }}"
                style="padding:8px 12px; border-radius:8px; background:#2563eb; color:white; text-decoration:none;">
                Inloggen
            </a>
            @if (Route::has('register'))
                <a href="{{ route('register') }}"
                    style="margin-left:8px; padding:8px 12px; border-radius:8px; background:#10b981; color:white; text-decoration:none;">
                    Registreren
                </a>
            @endif
        @endauth
...
<div style="max-width:960px; margin:0 auto; padding:24px;">
    ...
    @foreach($activiteiten as $a)
        <div style="border:1px solid #e5e7eb; padding:16px; margin:12px 0; border-radius:8px;">
            ...
            <p><strong>Tijd:</strong> {{ $a['begin_tijd'] }} – {{ $a['eind_tijd'] }}</p>
            <p><strong>Kosten:</strong> € {{ number_format($a['kosten'], 2, ',', '.') }}</p>
